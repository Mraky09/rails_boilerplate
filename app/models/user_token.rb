class UserToken < ApplicationRecord
  ...
  def renew!
    update_attributes!(token: secured_gen_str(:token), refresh_token: secured_gen_str(:refresh_token), expired_at: Time.zone.now + instance_eval(Settings.user_tokens.expired_period))
    self
  end
